...
export default function LandingPage() {
  ...
  const checkAuth = () => {
    ...
    // Bei authDisabled: Prüfe trotzdem, ob Token vorhanden ist für UI-State
    // Bei normaler Auth: Prüfe Token UND isAuthenticated
    const authenticated = authDisabled 
      ? !!token  // Bei disabled Auth: Nur prüfen ob Token vorhanden (für UI)
      : (token && auth.isAuthenticated());  // Bei normaler Auth: Token UND isAuthenticated
    
    console.log('[LandingPage] Auth check:', { 
      authenticated, 
      authDisabled, 
      hasToken: !!token,
      tokenValue: token ? `${token.substring(0, 10)}...` : 'none',
      isAuthenticatedResult: auth.isAuthenticated()
    });
  ...
  useEffect(() => {
    ...
    const handleAuthChange = () => {
      console.log('[LandingPage] Auth change event received');
      // Force immediate check mit kleiner Verzögerung für React State-Updates
      setTimeout(() => {
        checkAuth();
      }, 10);
    };
    
    // Listen for route changes (wenn User navigiert)
    const handleLocationChange = () => {
      console.log('[LandingPage] Location changed, rechecking auth');
      checkAuth();
    };
  ...
  // Zusätzlich: Prüfe Auth-Status bei Location-Change
  useEffect(() => {
    checkAuth();
  }, [location.pathname]);
  ...
  const handleLoginSuccess = () => {
    console.log('[LandingPage] Login success, updating auth state');
    checkAuth(); // Prüfe Auth-Status neu
    // Navigation zu /app (wird später implementiert, erstmal /groceries)
    navigate('/app');
  };
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            await auth.logout();
                            // Warte kurz, damit Event gefeuert werden kann
                            setTimeout(() => {
                              checkAuth(); // Prüfe Auth-Status neu
                              // Hard-Refresh für vollständigen Reset
